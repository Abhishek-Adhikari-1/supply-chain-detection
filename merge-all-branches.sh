...
if [ "$PUSH_TO_MAIN" = true ]; then
  log_info "Step 6: Merging integration branch to main..."
  
  if [ "$DRY_RUN" = true ]; then
    log_warn "DRY RUN: Would merge $INTEGRATION_BRANCH to main"
    git log --oneline origin/main..$INTEGRATION_BRANCH | head -10
  else
    ...
    if git merge origin/$INTEGRATION_BRANCH --no-ff -m "Merge $INTEGRATION_BRANCH: consolidate teammate work" 2>/dev/null; then
      log_success "Merged $INTEGRATION_BRANCH into main"
    else
      log_error "Conflict while merging to main"
      log_info "Resolving with -X theirs strategy..."
      git checkout --theirs .
      git add .
      git commit --no-edit -m "Merge $INTEGRATION_BRANCH: consolidate teammate work (resolved conflicts)"
      log_success "Merged with conflict resolution"
    fi
...
log_info "Integration branch: $INTEGRATION_BRANCH"
log_info "Push to main: $([ "$PUSH_TO_MAIN" = true ] && echo 'YES' || echo 'NO')"
...
echo "  2. Run tests: ./sandbox/test_all.sh (if available)"
[ "$PUSH_TO_MAIN" = false ] && echo "  3. Merge to main: $0 --push-main"
